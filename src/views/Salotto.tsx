...
export const SalottoView: React.FC<{ isDarkMode: boolean }> = ({ isDarkMode }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`h-full w-full p-4 overflow-y-auto ${isDarkMode ? 'bg-[#030712]' : 'bg-[#fff3ed]'}`}
    >
      ...
      <AnimatePresence>
        {selectedForum && (
          <motion.div
            ...
            className={`fixed inset-0 z-50 p-4 pt-12 ${isDarkMode ? 'bg-[#030712]' : 'bg-[#fff3ed]'}`}
          >
            <button 
