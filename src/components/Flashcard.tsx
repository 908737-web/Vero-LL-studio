    ...
    <div className="w-full space-y-2">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-4 bg-white/[0.02] rounded-2xl border border-white/5 mb-2 max-h-[10rem] overflow-y-auto custom-scrollbar">
              {children}
            </div>
          </motion.div>
        )}
    ...
    >
      ...
      <motion.div
        ...
        style={{ 
          ...
          // We combine the base rotation (0 or 180) with the tilt offset only when NOT flipped
      ...
      >
        {/* FRONT SIDE */}
        <div className={`absolute inset-0 backface-hidden border rounded-[2.5rem] overflow-hidden transition-colors ${isFlipped ? 'border-white/10' : 'border-transparent'}`}>
          <motion.div 
            className="absolute inset-0 bg-white/20 z-50 backdrop-blur-sm pointer-events-none"
          ...
          <GlassCard className="h-full flex flex-col p-0 border-0 rounded-none shadow-none">
            {/* Level Tag */}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-center gap-3">
                  <button 
                    ...
                    className={`w-8 h-8 rounded-full bg-white/[0.03] border border-white/10 flex items-center justify-center transition-all active:scale-90 ${
                        isSlowPlaying ? 'text-blue-400 border-blue-400/50 shadow-[0_0_15px_rgba(96,165,250,0.3)]' : 'text-white/20 hover:text-white/50 hover:bg-white/10'
                    }`}
                  >
                    <motion.div
        ...
        {/* BACK SIDE */}
        <div className={`absolute inset-0 backface-hidden rotate-y-180 border-white/10 border rounded-[2.5rem] overflow-hidden`}>
          <motion.div 
            className="absolute inset-0 bg-white/20 z-50 backdrop-blur-sm pointer-events-none"
            initial={{ clipPath: 'inset(0% 100% 0% 0%)' }}
            animate={isFlipped ? { clipPath: 'inset(0% 0% 0% 100%)' } : { clipPath: 'inset(0% 0% 0% 0%)' }}
          ...
          <GlassCard className="h-full flex flex-col p-4 px-5 border-0 rounded-none shadow-none bg-[#030712]/50 overflow-hidden">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar pr-1 mb-2 flex flex-col">
                {/* Core Info */}
                <div className="text-center space-y-1 mb-3 mt-4 shrink-0">
                    <div className="flex items-baseline justify-center gap-2">
                        <h2 className="text-white text-3xl font-bold tracking-tight farsi-text">{data.translation_fa}</h2>
                        <span className={`text-[0.625rem] font-black uppercase px-2 py-0.5 rounded-md ${
                            data.gender === Gender.MASCULINE ? 'bg-sky-500/20 text-sky-400' : 'bg-pink-500/20 text-pink-400'
                        }`}>
                            {data.gender_info_fa && data.gender_info_fa.includes('مذکر') ? 'IL' : 'LA'}
                        </span>
                    </div>
                    <div className="inline-flex glass px-3 py-1 rounded-full border-white/5 text-[0.625rem] font-bold text-white/30 uppercase tracking-tighter">
                       Means: <span className="text-white/60 ml-1">{data.translation_en}</span>
                    </div>
                </div>
                    ...
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="space-y-2">
                               ...
                               <p className="farsi-text text-white/40 text-[0.8125rem] leading-relaxed">{data.examples[0].fa}</p>
                            </div>
                        </div>
                    </Accordion>
                            ...
                            {data.dialogs.map((msg, i) => (
                                <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[85%] px-4 py-2.5 rounded-2xl shadow-lg ${
                                        ...
                                        <p className={`text-[0.625rem] ${msg.sender === 'user' ? 'text-white/60' : 'text-white/40'} farsi-text`}>{msg.fa}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    >
                        <div className="space-y-4">
                            <div className="space-y-2 bg-black/20 p-4 rounded-xl border border-white/5">
                                <p className="text-white/70 text-xs leading-relaxed leading font-medium italic">{data.story_it}</p>
                                <p className="farsi-text text-white/30 text-[0.6875rem] leading-relaxed leading">{data.story_fa}</p>
                            </div>
            ...
            <div className="shrink-0 flex flex-col justify-end pt-2 pb-1">
                <div className="flex justify-between space-x-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); triggerHaptic('medium'); handleAnswerWrapper('hard'); }}
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); triggerHaptic('medium'); handleAnswerWrapper('good'); }}
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); triggerHaptic('medium'); handleAnswerWrapper('easy'); }}
