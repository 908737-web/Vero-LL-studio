  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 scrollbar-hide">
              ...
              <div key={dIdx} className="mb-6 bg-slate-900/20 p-5 rounded-3xl border border-slate-800/50 shadow-lg">
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {def.synonyms.map(syn => (
                    <span key={syn} className="text-xs bg-emerald-900/20 text-emerald-400 px-3 py-1.5 rounded-full border border-emerald-900/30">مترادف: {syn}</span>
                  ))}
                  {def.antonyms.map(ant => (
                    <span key={ant} className="text-xs bg-rose-900/20 text-rose-400 px-3 py-1.5 rounded-full border border-rose-900/30">متضاد: {ant}</span>
                  ))}
                </div>
        ...
        {data.grammarNote && (
          <div className="mb-8 p-5 bg-blue-900/10 border border-blue-900/30 rounded-3xl flex gap-3">
            ...
            <p className="text-blue-100 text-sm leading-relaxed">
              <span className="font-bold text-blue-300 block mb-1">نکته گرامری:</span>
              {data.grammarNote}
            </p>
          </div>
        )}
        ...
        {/* Phrases */}
        <div className="mb-8">
          <h3 className="text-md font-bold text-white mb-4 flex items-center gap-2"><BookOpen size={18} className="text-indigo-400"/> اصطلاحات و عبارات</h3>
        ...
        {/* Nearby Words */}
        <div className="pb-10">
          <h3 className="text-xs font-bold text-slate-500 mb-4 uppercase tracking-wider">کلمات نزدیک</h3>
